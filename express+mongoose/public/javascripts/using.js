...
function fix (index) {
    ...
    useCase.placeholder = "使用情况:" + test[k].childNodes[7].innerHTML;
    evaluate.placeholder = "评价:" + test[k].childNodes[9].innerHTML;
...
(function (){
    ...
    form.onsubmit = function () {
        ...
        if(useCase.value == '' || useCase.value == null) {
             useCase.value =  testArr[2];
        }
        if(evaluate.value == '' || evaluate.value == null) {
             evaluate.value =  testArr[3];
        }
        form.action = '/fixUsing?id=' + testArr[4];
    }
})();
